...
\
    ...
    ASSERT_TRUE(!Universe::find_global(name)) << message;\
  }
#define checkClass(classHandle) {\
    ASSERT_TRUE(classHandle->as_oop()->is_mem()) << "Should be memOop";\
    ASSERT_TRUE(Universe::really_contains(classHandle->as_oop())) << "Should be in universe";\
  }
...
TEST_F(BehaviorPrimitivesSuperclassTests, setSuperclassShouldChangeSuperclassToNewClass) {
  ASSERT_TRUE(superclassOf(subclass()) == objectClass()) << "Original superclassHandle";

  oop result = behaviorPrimitives::setSuperclass(superclass(), subclass());

  ASSERT_TRUE(subclass() == result) << "Should return receiver";
  ASSERT_TRUE(superclassOf(subclass()) == superclass()) << "Superclass should have changed";
  ASSERT_TRUE(superclassOf(subclass()->klass()) == objectClass()->klass()) <<
...
TEST_F(BehaviorPrimitivesSuperclassTests, setSuperclassShouldChangeSuperclassToNil) {
  ...
  ASSERT_TRUE(subclass() == result) << "Should return receiver";
  ASSERT_TRUE(superclassOf(subclass()) == nilObj) << "Superclass should have changed";
  ASSERT_TRUE(superclassOf(subclass()->klass()) == objectClass()->klass()) <<
                "Metasuperclass should be unchanged";
}
...
TEST_F(BehaviorPrimitivesSuperclassTests, setSuperclassShouldChangeNilSuperclassToNotNil) {
  ...
  ASSERT_TRUE(superclassOf(subclass()) == objectClass()) << "Superclass should have changed back";
}
...
TEST_F(BehaviorPrimitivesSuperclassTests, setSuperclassShouldChangeTopSuperclassToClass) {
  ...
  ASSERT_TRUE(topClass()->klass() == result) << "Should return receiver";
  ASSERT_TRUE(superclassOf(topClass()->klass()) == classClass()) << "Superclass should have changed";
}

...
TEST(SystemPrimTests, expansionShouldExpandOldGenerationCapacity) {
  ...
  ASSERT_EQ(trueObj, systemPrimitives::expandMemory(as_smiOop(1000 * K))) << "Wrong result";
  ...
  ASSERT_EQ(expectedSize, actualSize) << msg;
}
...
TEST(SystemPrimTests, expansionShouldReturnMarkedResultWhenNotSMI) {
  ...
  ASSERT_TRUE(result->is_mark()) << "Result should be marked";
  ASSERT_EQ(vmSymbols::argument_has_wrong_type(), unmarkSymbol(result)) << "Wrong symbol";
}

TEST(SystemPrimTests, expansionShouldReturnMarkedResultWhenNegative) {
  oop result = systemPrimitives::expandMemory(as_smiOop(-1));
  ASSERT_TRUE(result->is_mark()) << "Result should be marked";
  ASSERT_EQ(vmSymbols::argument_is_invalid(), unmarkSymbol(result)) << "Wrong symbol";
}

TEST(SystemPrimTests, oopSize) {
  ASSERT_EQ(as_smiOop(oopSize), systemPrimitives::oopSize()) << "oopSize is wrong";
}
...
TEST(SystemPrimTests, expansionsShouldReturnExpansionCountAsSMI) {
  ...
  ASSERT_EQ(as_smiOop(expansions + 1), systemPrimitives::expansions()) << "Wrong expansion count";
}
...
TEST(SystemPrimTests, shrinkMemoryShouldReturnValueOutOfRangeWhenInsufficientFreeSpace) {
  int freeSpace = smiOop(systemPrimitives::freeSpace())->value();
  ASSERT_EQ((int)markSymbol(vmSymbols::value_out_of_range()), (int)systemPrimitives::shrinkMemory(as_smiOop(freeSpace + 1)));
  ASSERT_EQ(freeSpace, smiOop(systemPrimitives::freeSpace())->value());
}

TEST(SystemPrimTests, shrinkMemoryShouldReturnValueOutOfRangeWhenNegative) {
  int freeSpace = smiOop(systemPrimitives::freeSpace())->value();
  ASSERT_EQ((int)markSymbol(vmSymbols::value_out_of_range()), (int)systemPrimitives::shrinkMemory(as_smiOop(-1)));
  ASSERT_EQ(freeSpace, smiOop(systemPrimitives::freeSpace())->value());
}
...
TEST(SystemPrimTests, alienMallocShouldReturnAlignedValue) {
  ...
  ASSERT_TRUE(pointer->is_smi()) << "result should be SmallInteger";
  int address = smiOop(pointer)->value();
  ASSERT_TRUE(address % 4 == 0) << "not aligned";
  ASSERT_TRUE(address != 0) << "not allocated";
  free((void*)address);
}

TEST(SystemPrimTests, alienMallocShouldReturnMarkedSymbolWhenNotSmi) {
  oop pointer = systemPrimitives::alienMalloc(vmSymbols::abs());
  ASSERT_TRUE(pointer->is_mark()) << "return should be marked";
  ASSERT_EQ(markSymbol(vmSymbols::argument_has_wrong_type()), pointer) << "wrong symbol returned";
}
...
TEST(SystemPrimTests, alienMallocShouldReturnMarkedSymbolWhenSizeNegative) {
  ...
  ASSERT_TRUE(pointer->is_mark()) << "return should be marked";
  ASSERT_EQ(markSymbol(vmSymbols::argument_is_invalid()), pointer) << "wrong symbol returned";
}
...
TEST(SystemPrimTests, alienCallocContentsShouldBeZero) {
  ...
  for (int index = 0; index < 4; index++) {
    ...
    ASSERT_EQ((int)address[index], 0) << message;
  }
  free((void*)address);
}
...
TEST(SystemPrimTests, alienFreeShouldReturnMarkedSymbolWhenNotSmi) {
  ...
  ASSERT_TRUE(result->is_mark()) << "return should be marked";
  ASSERT_EQ(markSymbol(vmSymbols::argument_has_wrong_type()), result) << "wrong symbol returned";
}
...
TEST(SystemPrimTests, alienCallocShouldReturnMarkedSymbolWhenAddressZero) {
  ...
  ASSERT_TRUE(result->is_mark()) << "return should be marked";
  ASSERT_EQ(markSymbol(vmSymbols::argument_is_invalid()), result) << "wrong symbol returned";
}
...
TEST(SystemPrimTests, alienFreeShouldFreeLargeIntegerAddress) {
  ...
  ASSERT_TRUE(!result->is_mark()) << "should not be marked";
  ASSERT_TRUE(result == trueObj) << "result should be true";
}
